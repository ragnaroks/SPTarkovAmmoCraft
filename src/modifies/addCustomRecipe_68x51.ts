...
const newId: string = '6826669d350644dc41a23100';

const stuffsTemplateArray:Array<ItemTpl> = [
  ItemTpl.AMMO_68X51_FMJ,
];

export default function addCustomRecipe_68x51(logger: ILogger,tables: IDatabaseTables) {
  const productTemplate = tables.templates.items[ItemTpl.AMMO_68X51_HYBRID] || null;
  if(!productTemplate){
    logger.error('[SPTarkovAmmoCraft]：addCustomRecipe_68x51，Error：template AMMO_68X51_HYBRID not found');
    return;
  }
  ...
  for (const id of stuffsTemplateArray) {
    ...
    const newRecipe:IHideoutProduction = {
      ...
      productionTime: 3600,
      endProduct: ItemTpl.AMMO_68X51_HYBRID,
      isEncoded:false,
      locked: false,
    ...
    logger.debug('[SPTarkovAmmoCraft]：addCustomRecipe_68x51，ID：' + newRecipe._id);
  }

  logger.success('[SPTarkovAmmoCraft]：addCustomRecipe_68x51，ID：' + newId);
}
