...
const newId: string = '682666ffa2339fe2a0ac5c00';

const stuffsTemplateArray:Array<ItemTpl> = [
  ItemTpl.AMMO_762X35_CBJ,ItemTpl.AMMO_762X35_M62,ItemTpl.AMMO_762X35_BCP_FMJ,ItemTpl.AMMO_762X35_VMAX,
  ItemTpl.AMMO_762X35_WHISPER
];

export default function addCustomRecipe_762x35(logger: ILogger,tables: IDatabaseTables) {
  const productTemplate = tables.templates.items[ItemTpl.AMMO_762X35_AP] || null;
  if(!productTemplate){
    logger.error('[SPTarkovAmmoCraft]：addCustomRecipe_762x35，Error：template AMMO_762X35_AP not found');
    return;
  }
  ...
  for (const id of stuffsTemplateArray) {
    ...
    const newRecipe:IHideoutProduction = {
      ...
      productionTime: 3600,
      endProduct: ItemTpl.AMMO_762X35_AP,
      isEncoded:false,
      locked: false,
    ...
    logger.debug('[SPTarkovAmmoCraft]：addCustomRecipe_762x35，ID：' + newRecipe._id);
  }

  logger.success('[SPTarkovAmmoCraft]：addCustomRecipe_762x35，ID：' + newId);
}
