...
const newId: string = '68266dcba339e932b7bedb00';

const stuffsTemplateArray:Array<ItemTpl> = [
  ItemTpl.AMMO_762X54R_SNB,ItemTpl.AMMO_762X54R_BT,ItemTpl.AMMO_762X54R_PS,ItemTpl.AMMO_762X54R_LPS,
  ItemTpl.AMMO_762X54R_T46M,ItemTpl.AMMO_762X54R_FMJ,ItemTpl.AMMO_762X54R_SP_BT,ItemTpl.AMMO_762X54R_HP_BT
];

export default function addCustomRecipe_762x54(logger: ILogger,tables: IDatabaseTables) {
  const productTemplate = tables.templates.items[ItemTpl.AMMO_762X54R_BS] || null;
  if(!productTemplate){
    logger.error('[SPTarkovAmmoCraft]：addCustomRecipe_762x54，Error：template AMMO_762X54R_BS not found');
    return;
  }
  ...
  for (const id of stuffsTemplateArray) {
    ...
    const newRecipe:IHideoutProduction = {
      ...
      productionTime: 3600,
      endProduct: ItemTpl.AMMO_762X54R_BS,
      isEncoded:false,
      locked: false,
    ...
    logger.debug('[SPTarkovAmmoCraft]：addCustomRecipe_762x54，ID：' + newRecipe._id);
  }

  logger.success('[SPTarkovAmmoCraft]：addCustomRecipe_762x54，ID：' + newId);
}
