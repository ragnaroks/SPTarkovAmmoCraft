...
const newId: string = '682667dabb66300840ae7900';

const stuffsTemplateArray:Array<ItemTpl> = [
  ItemTpl.AMMO_86X70_FMJ,ItemTpl.AMMO_86X70_UCW,ItemTpl.AMMO_86X70_TACX,
];

export default function addCustomRecipe_86x70(logger: ILogger,tables: IDatabaseTables) {
  const productTemplate = tables.templates.items[ItemTpl.AMMO_86X70_AP] || null;
  if(!productTemplate){
    logger.error('[SPTarkovAmmoCraft]：addCustomRecipe_86x70，Error：template AMMO_86X70_AP not found');
    return;
  }
  ...
  for (const id of stuffsTemplateArray) {
    ...
    const newRecipe:IHideoutProduction = {
      ...
      productionTime: 3600,
      endProduct: ItemTpl.AMMO_86X70_AP,
      isEncoded:false,
      locked: false,
    ...
    logger.debug('[SPTarkovAmmoCraft]：addCustomRecipe_86x70，ID：' + newRecipe._id);
  }

  logger.success('[SPTarkovAmmoCraft]：addCustomRecipe_86x70，ID：' + newId);
}
