...
const newId: string = '68266c730fa5dd131c4f7500';

const stuffsTemplateArray:Array<ItemTpl> = [
  ItemTpl.AMMO_366TKM_EKO,ItemTpl.AMMO_366TKM_FMJ,ItemTpl.AMMO_366TKM_GEKSA,
];

export default function addCustomRecipe_955x39(logger: ILogger,tables: IDatabaseTables) {
  const productTemplate = tables.templates.items[ItemTpl.AMMO_366TKM_APM] || null;
  if(!productTemplate){
    logger.error('[SPTarkovAmmoCraft]：addCustomRecipe_955x39，Error：template AMMO_366TKM_APM not found');
    return;
  }
  ...
  for (const id of stuffsTemplateArray) {
    ...
    const newRecipe:IHideoutProduction = {
      ...
      productionTime: 3600,
      endProduct: ItemTpl.AMMO_366TKM_APM,
      isEncoded:false,
      locked: false,
    ...
    logger.debug('[SPTarkovAmmoCraft]：addCustomRecipe_955x39，ID：' + newRecipe._id);
  }

  logger.success('[SPTarkovAmmoCraft]：addCustomRecipe_955x39，ID：' + newId);
}
