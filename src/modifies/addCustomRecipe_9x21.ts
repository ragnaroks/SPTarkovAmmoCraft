...
const newId: string = '68265c893b91a5b8a0cc7200';

const stuffsTemplateArray:Array<ItemTpl> = [
  ItemTpl.AMMO_9X21_BT,ItemTpl.AMMO_9X21_7U4,ItemTpl.AMMO_9X21_PS,ItemTpl.AMMO_9X21_P,ItemTpl.AMMO_9X21_PE
];

export default function addCustomRecipe_9x21(logger: ILogger,tables: IDatabaseTables) {
  const productTemplate = tables.templates.items[ItemTpl.AMMO_9X21_7N42] || null;
  if(!productTemplate){
    logger.error('[SPTarkovAmmoCraft]：addCustomRecipe_9x21，Error：template AMMO_9X21_7N42 not found');
    return;
  }
  ...
  for (const id of stuffsTemplateArray) {
    ...
    const newRecipe:IHideoutProduction = {
      ...
      productionTime: 3600,
      endProduct: ItemTpl.AMMO_9X21_7N42,
      isEncoded:false,
      locked: false,
    ...
    logger.debug('[SPTarkovAmmoCraft]：addCustomRecipe_9x21，ID：' + newRecipe._id);
  }

  logger.success('[SPTarkovAmmoCraft]：addCustomRecipe_9x21，ID：' + newId);
}
