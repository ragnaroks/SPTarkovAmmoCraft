...
const newId: string = '68265f7999800c366131de00';

const stuffsTemplateArray:Array<ItemTpl> = [
  ItemTpl.AMMO_9X33R_JHP,ItemTpl.AMMO_9X33R_HP,ItemTpl.AMMO_9X33R_SP
];

export default function addCustomRecipe_9x33r(logger: ILogger,tables: IDatabaseTables) {
  const productTemplate = tables.templates.items[ItemTpl.AMMO_9X33R_FMJ] || null;
  if(!productTemplate){
    logger.error('[SPTarkovAmmoCraft]：addCustomRecipe_9x33r，Error：template AMMO_9X33R_FMJ not found');
    return;
  }
  ...
  for (const id of stuffsTemplateArray) {
    ...
    const newRecipe:IHideoutProduction = {
      ...
      productionTime: 3600,
      endProduct: ItemTpl.AMMO_9X33R_FMJ,
      isEncoded:false,
      locked: false,
    ...
    logger.debug('[SPTarkovAmmoCraft]：addCustomRecipe_9x33r，ID：' + newRecipe._id);
  }

  logger.success('[SPTarkovAmmoCraft]：addCustomRecipe_9x33r，ID：' + newId);
}
